...
- [x] Add `gomod` and `npm` to Dependabot: the dependency drift recorded at the top of this file (Wails alpha.72 vs beta.23, `@wailsio/runtime` mismatched, Angular 21.2 vs 22.1) is exactly what those ecosystems would surface. Monthly, grouped by family, majors proposed (unlike actions); Wails is excluded on both sides and stays a manual, paired bump
