...
- [x] Act on what the Fix eval found (#80): the input is delimited as a document and the output contract is stated, plus a deterministic guard in `enhance.Service`. Both metrics improved with non-overlapping ranges on three runs — deterministic 0.9263 → 0.9513, judge 0.8520 → 0.9453 — and two of the three violations are gone in every run. The third (translating a deliberate code-switch) is not fixed and stays on #80; strengthening rule 4 was measured across four prompt revisions, never stopped it, and suppressed rule 8's single-word replacement instead. Prompt hashes and intervals in [`docs/fix/quality-status.md`](fix/quality-status.md)
