...
- [x] Re-baseline on **Opus 5** — done 2026-09-25 (`quality-status.md`, three runs each): Sonnet 5 deterministic clearly up, judge inconclusive; Opus 5 deterministic 0.888, clearly above both, judge 0.820 — below Sonnet 4.6 with disjoint ranges. Both newer models trade judge score for deterministic score on the shipped v2 prompt; the default-model decision (ADR-002) now has all three rows
