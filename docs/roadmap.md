...
- [x] Act on what the Fix eval found (#80): the input is delimited as a document and the output contract is stated, plus a deterministic output guard in `enhance.Service`. Two of the three violations are gone in all three runs — the model no longer answers a chat message (judge 0.25 → 1.00 on that sample) and no longer annotates correct text (0.30 → 1.00) — and the guard recorded **zero interventions in 45 samples**, so those are the prompt's doing. Judge overall 0.8520 → 0.9524 with disjoint ranges; deterministic 0.9263 → 0.9465 **overlaps**, and three samples that used to pass no longer do (`markdown-struktur` 3/3 → 0/3). The third violation is unfixed and stays on #80. Numbers, the failed attempts, and the selection bias in the six-variant search are in [`docs/fix/quality-status.md`](fix/quality-status.md)
- [ ] **Before the next Fix prompt change: a held-out split.** Six variants were measured on the same fifteen samples and the winner re-measured 0.005 lower with one fewer sample passing — the search itself moves the number. Ten samples to tune against, five untouched until the end
